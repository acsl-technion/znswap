...
int clear_page_dirty_for_io(struct page *page)
{
	...
	if (PageSwapBacked(page) && PageSwapCache(page) && page_is_tmp_zns(page)) {
		...
		if (page_mkpteclean(page))
			set_page_dirty(page);
#endif
		VM_BUG_ON(!PageDirty(page));
	}
#ifdef CONFIG_DEBUG_VM
	else if (PageSwapBacked(page) && PageSwapCache(page) && page_is_zns(page)){
		BUG();
	}
#endif
	else if (mapping && mapping_can_writeback(mapping)) {
		struct inode *inode = mapping->host;
		struct bdi_writeback *wb;
		struct wb_lock_cookie cookie = {};
...
int test_clear_page_writeback(struct page *page)
{
	...
	if (!(PageSwapBacked(page) && PageSwapCache(page) && page_is_tmp_zns(page)) &&
			(mapping && mapping_use_writeback_tags(mapping))) {
		struct inode *inode = mapping->host;
		struct backing_dev_info *bdi = inode_to_bdi(inode);
		unsigned long flags;
