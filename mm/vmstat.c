...
static int swapstat_proc_show(struct seq_file *m, void *v)
{
	char buf[512];
	long nr_swapslots, nr_swapclusters;
	nr_swapslots = total_swap_pages - atomic_long_read(&nr_swap_pages);
	nr_swapclusters = atomic_long_read(&nr_swap_clusters);

	if (nr_swapslots < 0 || nr_swapclusters < 0)
		pr_info("something wrong with swap counters");

	sprintf(buf,"nr_total_swapslots %lu\n"
			"nr_swapslots %lu\n"
			"nr_total_swapclusters %lu\n"
			"nr_swapclusters %lu",
			total_swap_pages,
			nr_swapslots,
			total_swap_clusters,
...
void __init init_mm_internals(void)
{
	...
	proc_create_single("swapstat", 0444, NULL, swapstat_proc_show);
